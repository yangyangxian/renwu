...
export const userSyncQueue = appConfig.redisEnabled
  ? new Queue('user-sync', { connection: { url: appConfig.redisUrl } })
  : undefined;
...
export async function scheduleUserSyncJob() {
  if (!appConfig.redisEnabled || !redisClient || !userSyncQueue) {
    return;
  }
