...
export function shouldRenderTimelineCalendar(groupCount: number) {
  return groupCount > 0;
}
...
export default function TimelineView({
  ...
  const selectedDate = useMemo(() => selectedDateKey ? parseTimelineDateKey(selectedDateKey) : undefined, [selectedDateKey]);
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 px-1">
              <div className="flex min-w-0 items-center gap-3">
                ...
                <div className="flex flex-wrap items-center gap-x-3 gap-y-1">
                  ...
                  <span className="text-sm font-medium text-muted-foreground dark:text-foreground">{selectedDate ? formatWeekday(selectedDate) : ''}</span>
                </div>
              </div>
            </div>
          ...
          <div className="space-y-3">
            {months.map((month) => (
              <Card
                ...
                className="w-fit max-w-full rounded-md border border-border/70 bg-background/90 px-3.5 py-3 shadow-xs"
              >
                <Calendar
                  mode="single"
                  month={month.date}
